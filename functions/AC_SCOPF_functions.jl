...
function add_investment_variables(model_name)
    @variable(model_name, lines_S_reinforcement[l = 1:nLines], lower_bound=0.0, upper_bound = array_lines_reinf[l].line_Smax_A)
    # @variable(model_name, trans_S_reinforcement[t = 1:nTrans], lower_bound=0.0)
return lines_S_reinforcement
end
            ...
            for t in 1:nTP
                ...
                if  ~isempty(node_data[i,1].node_num)
                    ...
                    for j in 1:size(node_data[i,1].node_num,1)                                                # length of each node vector in 'node_data' variable
                        ...
                        # # cleaning up to free up memory for subsequent iterations:
                        # pij = nothing
                        # qij = nothing
                    end
               end
...
(gij_line_c^2+bij_line_c^2)*((e_c[c,s,t,i]-e_c[c,s,t,idx_cnode_c])^2 + (f_c[c,s,t,i]-f_c[c,s,t,idx_cnode_c])^2 )<=(node_smax_c[c][i].node_smax_c[j])^2
)
...
if Inv_opt == 0
    ...
    function line_flow_n(model_name,pinj_dict,qinj_dict,full_indicator )
        ...
        line_flow_normal_s=@NLconstraint(model_name, [t in 1:nTP, l in 1:nLines],
    (pinj_dict[[t,idx_from_line[l],idx_to_line[l]]])^2+(qinj_dict[[t,idx_from_line[l],idx_to_line[l]]])^2<=(nw_lines[l].line_Smax_A + lines_S_reinforcement[l])^2
                                            )
    ...
    (pinj_dict[[t,idx_from_trans[tr],idx_to_trans[tr]]])^2+(qinj_dict[[t,idx_from_trans[tr],idx_to_trans[tr]]])^2<=(nw_trans[tr].trans_Snom)^2
            ...
            if full_indicator=="full"
    line_flow_normal_r=@NLconstraint(model_name, [ t in 1:nTP, l in 1:nLines],
        (pinj_dict[[t,idx_to_line[l],idx_from_line[l]]])^2+(qinj_dict[[t,idx_to_line[l],idx_from_line[l]]])^2<=(nw_lines[l].line_Smax_A + lines_S_reinforcement[l])^2
                                        )
    line_flow_normal_trans_r=@NLconstraint(model_name, [t in 1:nTP, tr in 1:nTrans],
    (pinj_dict[[t,idx_to_trans[tr],idx_from_trans[tr]]])^2+(qinj_dict[[t,idx_to_trans[tr],idx_from_trans[tr]]])^2<=(nw_trans[tr].trans_Snom)^2
                                        )
...
if Inv_opt == 0
    ...
    function line_flow_c(model_name,pinj_dict_c,qinj_dict_c,full_indicator )
    global line_number_indicator = round.(Int, zeros(nBus,nBus)) # auxiliary matrix to track how contingency lines correspond to the original lines numbering (i=from, j=to)
    for ll = 1:size(nw_lines)[1]
        line_number_indicator[nw_lines[ll].line_from, nw_lines[ll].line_to] = ll
    end

        line_flow_contin_s=@NLconstraint(model_name, [c in 1:nCont, s in 1:nSc, t in 1:nTP, l in 1:length(idx_from_line_c[c])],
                # (pinj_dict_c[[c,s,t,b,node_data_contin[c][b].node_cnode_c[j]]])^2+(qinj_dict_c[[c,s,t,b,node_data_contin[c][b].node_cnode_c[j]]])^2<=(node_data_contin[c][b].node_smax_c[j])^2
                # (pinj_dict_c[[c,s,t,idx_from_line_c[c][l],idx_to_line_c[c][l]]])^2+(qinj_dict_c[[c,s,t,idx_from_line_c[c][l],idx_to_line_c[c][l]]])^2<=(line_smax_c[c][l])^2
                (pinj_dict_c[[c,s,t,idx_from_line_c[c][l],idx_to_line_c[c][l]]])^2+(qinj_dict_c[[c,s,t,idx_from_line_c[c][l],idx_to_line_c[c][l]]])^2<=(line_smax_c[c][l] + lines_S_reinforcement[line_number_indicator[idx_from_line_c[c][l], idx_to_line_c[c][l]]])^2
                )
        line_flow_contin_trans_s=@NLconstraint(model_name, [c in 1:nCont, s in 1:nSc, t in 1:nTP, tr in 1:nTrans],
    (pinj_dict_c[[c,s,t,idx_from_trans[tr],idx_to_trans[tr]]])^2+(qinj_dict_c[[c,s,t,idx_from_trans[tr],idx_to_trans[tr]]])^2<=(nw_trans[tr].trans_Snom)^2
                )
        ...
        line_flow_contin_r=@NLconstraint(model_name, [c in 1:nCont, s in 1:nSc, t in 1:nTP, l in 1:length(idx_from_line_c[c])],
            ...
            # (pinj_dict_c[[c,s,t,b,node_data_contin[c][b].node_cnode_c[j]]])^2+(qinj_dict_c[[c,s,t,b,node_data_contin[c][b].node_cnode_c[j]]])^2<=(node_data_contin[c][b].node_smax_c[j])^2
            # (pinj_dict_c[[c,s,t,idx_to_line_c[c][l],idx_from_line_c[c][l]]])^2+(qinj_dict_c[[c,s,t,idx_to_line_c[c][l],idx_from_line_c[c][l]]])^2<=(line_smax_c[c][l])^2
            (pinj_dict_c[[c,s,t,idx_to_line_c[c][l],idx_from_line_c[c][l]]])^2+(qinj_dict_c[[c,s,t,idx_to_line_c[c][l],idx_from_line_c[c][l]]])^2<=(line_smax_c[c][l] + lines_S_reinforcement[line_number_indicator[idx_from_line_c[c][l], idx_to_line_c[c][l]]])^2
            )
        line_flow_contin_trans_r=@NLconstraint(model_name, [c in 1:nCont, s in 1:nSc, t in 1:nTP, tr in 1:nTrans],
    (pinj_dict_c[[c,s,t,idx_to_trans[tr],idx_from_trans[tr]]])^2+(qinj_dict_c[[c,s,t,idx_to_trans[tr],idx_from_trans[tr]]])^2<=(nw_trans[tr].trans_Snom)^2
                )
...
lsh_aux=@expression(model_name,
[i=1:nBus ; ~isempty(findall(x->x==i,bus_data_lsheet)) ],
sum(pen_lsh[t,findall(x->x==i,bus_data_lsheet)] for t in 1:nTP)
        )
lsh=@expression(model_name,sum(lsh_aux))
...
lsh_aux_c=@expression(model_name,
                   [i=1:nBus ; ~isempty(findall(x->x==i,bus_data_lsheet)) ],
                   sum(pen_lsh_c[c,s,t,findall(x->x==i,bus_data_lsheet)] for c in 1:nCont,s in 1:nSc,t in 1:nTP)
                             )
lsh_c=@expression(model_name,sum(lsh_aux_c))
...
if Inv_opt == 0
    if Obj_f_opt == 0 || Obj_f_opt == 2
        @objective(model_name,Min,total_cost)
    elseif Obj_f_opt == 1
        @objective(model_name,Min,sum(lsh)+sum(lsh_c))
    end
else
    if Obj_f_opt == 0 || Obj_f_opt == 2
        @objective(model_name,Min,total_cost + sum(lines_S_reinforcement[l]*lines_inv_costs[l] for l = 1:nLines))
    elseif Obj_f_opt == 1
        @objective(model_name,Min,sum(lsh)+sum(lsh_c))
    end
end
...
function SP_SCOPF_or_MP_OPF(model_indicator)
    ...
    if Inv_opt == 1
        lines_S_reinforcement = add_investment_variables(model_name)
        global lines_S_reinforcement = lines_S_reinforcement
    end
   ...
   println("Number of buses (nBus): ",nBus)
   println("Number of lines (nLines): ",nLines)
   println("Number of transformers (nTrans): ",nTrans)
   println("Total number of branches: ",nLines+nTrans)
   println("size(prof_ploads): ",size(prof_ploads))
   println("Time periods (nTP): ",nTP)
   println("Number of scenarios (nSc): ",nSc)
   println("Number of contingencies (nCont): ",nCont)
   println("Number of loads (nLoads): ",nLoads)
   println("Number of RES (nRES): ",nRES)
   totalP_check = 0
   totalQ_check = 0
   for t in 1:nTP
       for b in 1:nBus
   ...
   println("Total P gen_max capacity: ",sum(pg_max))
   println("Total Q gen_max capacity: ",sum(qg_max))
   ...
   println("")
    ...
    println("JuMP.termination_status: ",JuMP.termination_status(model_name))
   ...
   # # cleaning up to free up memory for subsequent iterations:
#    global pinj_dict = nothing
#    global qinj_dict = nothing
#    global pinj_dict_c = nothing
#    global qinj_dict_c = nothing
#    global active_power_balance_normal = nothing
#    global active_power_balance_contin = nothing
#    global reactive_power_balance_normal = nothing
#    global reactive_power_balance_contin = nothing
#    global line_flow_normal_s = nothing
#    global line_flow_normal_r = nothing
#    global line_flow_normal_trans_s = nothing
#    global line_flow_normal_trans_r = nothing
#    global line_flow_contin_s = nothing
#    global line_flow_contin_r = nothing
#    global line_flow_contin_trans_s = nothing
#    global line_flow_contin_trans_r = nothing
#    global total_cost = nothing
#    global cost_gen = nothing
#    global cost_pen_lsh = nothing
#    global cost_fl = nothing
#    global cost_str = nothing
#    global cost_pen_ws = nothing
#    global cost_pen_lsh_c = nothing
#    global cost_pen_ws_c = nothing
#    global cost_fl_c = nothing
#    global cost_str_c = nothing

#    global e = nothing
#    global f = nothing
#    global Pg = nothing
#    global Qg = nothing
#    global pen_ws = nothing
#    global pen_lsh = nothing
#    global p_fl_inc = nothing
#    global p_fl_dec = nothing
#    global q_fl_inc = nothing
#    global q_fl_dec = nothing
#    global p_ch = nothing
#    global p_dis = nothing
#    global soc = nothing
#    global Pg_neg = nothing
#    global Qg_neg = nothing

#    global  e_c = nothing
#    global  f_c = nothing
#    global  Pg_c = nothing
#    global  Qg_c = nothing
#    global  pen_ws_c = nothing
#    global  pen_lsh_c = nothing
#    global  p_fl_inc_c = nothing
#    global  p_fl_dec_c = nothing
#    global  q_fl_inc_c = nothing
#    global  q_fl_dec_c = nothing
#    global  p_ch_c = nothing
#    global  p_dis_c = nothing
#    global  soc_c = nothing
#    global  Pg_neg_c = nothing
#    global  Qg_neg_c = nothing
