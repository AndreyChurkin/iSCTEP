...
u = Shapley_matrix(nPl) # get the scenarios (possible coalitions)
...
function coalition_JSON_update(json_file_path,coalition)
...
for coalition = 1:size(u)[1]
    ...
    println()
    json_file_path = "data_preparation\\import_WP3.json" # file to update for each coalition

    try # try/catch in case JSON file is still busy
        coalition_JSON_update(json_file_path,coalition)
    catch # wait (secs) and try again
        sleep(5)
        coalition_JSON_update(json_file_path,coalition)
    end
...
# CSV.write("coalitional_analysis_output_C5_RES_operation_Vfunction_max_plc.csv",  DataFrame(Names=("Vfunction_max_plc"), Vector=Vfunction_max_plc))
# CSV.write("coalitional_analysis_output_C5_RES_operation_Vfunction_max_qlc.csv",  DataFrame(Names=("Vfunction_max_qlc"), Vector=Vfunction_max_qlc))
# CSV.write("coalitional_analysis_output_C5_RES_operation_Vfunction_avoided_cost.csv",  DataFrame(Names=("Vfunction_avoided_cost"), Vector=Vfunction_avoided_cost))
# CSV.write("coalitional_analysis_output_C5_RES_operation_coalition_term_status.csv",  DataFrame(Names=("coalition_term_status"), Vector=coalition_term_status))
# CSV.write("coalitional_analysis_output_C5_RES_operation_coalition_elapsed_time.csv",  DataFrame(Names=("coalition_elapsed_time"), Vector=coalition_elapsed_time))
...
# figure_name = "C5 - violin plots"
# savefig(figure_name*".svg")
# savefig(figure_name*".png")

# figure_name = "C5 - violin plots - total cost operation test2"
# savefig(figure_name*".svg")
# savefig(figure_name*".png")

# figure_name = "C5 - violin plots - total expected cost test3"
# savefig(figure_name*".svg")
# savefig(figure_name*".png")
